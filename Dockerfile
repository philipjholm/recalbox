...
CMD echo "${RECALBOX_VERSION_LABEL}" > board/recalbox/fsoverlay/recalbox/recalbox.version ; ( cd buildroot && git reset HEAD --hard && git clean -dfx ) && make recalbox-${ARCH}_defconfig && [ "$RECALBOX_CCACHE_DIR" != "" ] && RECALBOX_CCACHE="BR2_CCACHE=y" ; BR2_DL_DIR="/share/dl" BR2_CCACHE_DIR="$RECALBOX_CCACHE_DIR" make BR2_HOST_DIR=/share/host $RECALBOX_CCACHE BR2_CCACHE_INITIAL_SETUP="--max-size=5G" BR2_CCACHE_USE_BASEDIR=y
